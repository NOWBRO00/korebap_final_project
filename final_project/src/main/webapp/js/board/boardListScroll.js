						...
						boardList.forEach((board, i) => {
							...
							<div class="col-md-4">
							       ${board.board_file_dir
							           ? (board.board_file_dir.startsWith('http')
							               ? `<img src="${board.board_file_dir}" alt="게시글 이미지" class="blog-item set-bg">`
							               : `<img src="img/board/${board.board_file_dir}" alt="게시글 이미지" class="blog-item set-bg">`)
							           : `<img src="img/board/boardBasic.png" alt="기본 이미지" class="blog-item set-bg">`
							       }
							       <div class="blog-item scroll_counter" style="height: 120px;">
							           <div class="bi-text" style="padding-left: 0px; bottom: 2px; position: relative;">
							               <span class="b-tag">자유게시판</span>
							               <span class="b-tag">${board.board_like_cnt}</span>
							               <h4>
							                   <a class="text-dark" href="boardDetail.do?member_id=${board.member_id}&board_num=${board.board_num}">
							                       ${board.board_title}
							                   </a>
							               </h4>
							               <div class="b-time">
